...
for line in lines:
    if 'lr :' in line:
        ...
        if match:
            list_loss.append(float(match.group(1)))
        # print(line)
        # break
print(len(list_loss))
...
x = np.arange(iters_num-20*100, iters_num, 20)  ################################ 自己估计下坐标轴x,这里10是源代码默认iter=10输出一次loss
plt.plot(x, list_loss[-100:], label="Total Loss")
