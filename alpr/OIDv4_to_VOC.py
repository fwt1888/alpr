...
ids = []
for file in os.listdir(args.sourcepath.replace("\"","")): #Save all images in a list
    filename = os.fsdecode(file)
    if filename.endswith('.jpg'):
        ids.append(filename[:-4])

for fname in ids: 
    myfile = os.path.join(args.dest_path.replace("\"",""),fname +'.xml')
    myfile = Path(myfile)
    if not myfile.exists(): #if file is not existing 
        txtfile = os.path.join(args.sourcepath.replace("\"",""), 'Label', fname + '.txt') #Read annotation of each image from txt file
        f = open(txtfile,"r")
        imgfile = os.path.join(args.sourcepath.replace("\"",""), fname +'.jpg')
        ...
        for x in f:     #Iterate for each object in a image. 
            ...
            name = []
            for item in x:
                if item.isalpha():  # 判断是否为英文字母
                    name.append(item)
            print(name)
            print(len(name))

            child_name = SubElement(child_obj, 'name')
            child_name.text = " ".join(name) #name
            ...
            child_xmin.text = str(int(float(x[len(name)]))) #xmin

            child_ymin = SubElement(child_bndbox, 'ymin')
            child_ymin.text = str(int(float(x[len(name) + 1]))) #ymin

            child_xmax = SubElement(child_bndbox, 'xmax')
            child_xmax.text = str(int(float(x[len(name) + 2]))) #xmax

            child_ymax = SubElement(child_bndbox, 'ymax')
            child_ymax.text = str(int(float(x[len(name) + 3]))) #ymax
